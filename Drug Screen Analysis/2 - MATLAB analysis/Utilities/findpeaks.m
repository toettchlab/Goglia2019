function [Ypk,Xpk,Wpk,Ppk,wxPk] = findpeaks(Yin,varargin)
...
% use the index vector to fecth the correct peaks.
iPk = iPk(idx);
if needWidth
...
% merge finite and infinte peaks together into one list
...
% pre-initialize output base indices
iBase = zeros(size(iPeak));

% table stores the most recently encoutered peaks in order of height
peak = zeros(size(iPeak));
valley = zeros(size(iPeak));
iValley = zeros(size(iPeak));
...
% auto", save the y limits, set axis to "tight" then restory the y limits.
...
if ~isempty(y) && ~isempty(hLine) && ishghandle(hLine)
  ...
  if isempty(cbActionPostZoom) || isequal(char(cbActionPostZoom),char(cbZoom))
    ...
    e = events(hFig);
    eventName = e{strncmp(e,'SizeChange',10)};
    listener = matlab.ui.internal.createListener(hFig, eventName, cbSizeChange);
